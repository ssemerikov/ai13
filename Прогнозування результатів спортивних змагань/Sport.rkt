...
(define NUMHID 3) ;размерность скрытого слоя
...
(define Рамка (new frame%       [label "Прогнозування результатів спортивних змагань"])) 
 ; створимо повідомлення на рамці
(define Повідомлення (new message%       [parent Рамка] 

 
      [label "                                Що вам потрібно?                                "])) 
 ; створимо горизонтальну панель, на якій
; розмістимо кнопки з заголовками "Ліва" та "Права"
(define Панель (new horizontal-panel%       [parent Рамка]       [alignment '(center center)]))
...
(define Поле1 (new text-field%
                  [label "Введіть тур :"]
                  [parent Рамка] ))
(define operation1 (new choice% [parent Рамка] 
[label "Введіть 1 команду: "] 
[choices '("Барс" "Витязь" "Кривбас" "Кременчук" "Донбасс" "Дженералз" )]))
(define operation2 (new choice% [parent Рамка] 
[label "Введіть 2 команду: "] 
[choices '("Барс" "Витязь" "Кривбас" "Кременчук" "Донбасс" "Дженералз" )]))
...
[min-width 200]
[callback (lambda (button event)
            (define x1 (string->number (send Поле1 get-value)))
            (define op1 (send operation1 get-selection))
            (define op2 (send operation2 get-selection))
            (define res 
(begin
  (set! z (list->vector (append (list (abs x1)) (list (abs (+ 1 op1))) (list (abs ( + 1 op2))))))
  (send Повідомлення set-label (format "~a" z ) ) ))
...
(define (compute)
  (if (= 0 NUMHID)
      ...
      ;  (readvector in)
        (set! res (getoutput z))
        (newline)
        (message-box "Полученный выход :" (format  "Команда 1 переможе  з ймовірністю : ~a"  (car (vector->list res))) #f '(ok))
