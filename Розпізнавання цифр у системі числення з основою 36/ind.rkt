...
(define NUMHID 36) ;размерность скрытого слоя
  ...
  (let ((Error 0) (p 0) 
    ...
    (do ((epoch 0 (1+ epoch))) ((or (= epoch MaxCount) (< Error Err)) Error)
      ...
      (when (and DoOut (= 0 (remainder epoch 1000))) ;отладочный вывод
        (display (format "Итерація=~a, помилка=~a" epoch Error))
        (newline)
        )
...
(define (menu)
  (display "Оберіть бажаний пункт:")
  (newline)
  (display "1. Навчання нейронної мережі (ДОВГО!!!)")
  (newline)
  (display "2. Розпізнавання")
  (newline)
  (display "3. Вихід")
  (newline)
  (define choice (read))
  (cond
    ((= choice 1) (main) (menu))
    ((= choice 2) (compute) (menu))
    ((= choice 3) )
    (else  (and (display "Неправильне значення") (newline)(menu)))
    )
  )
...
(define (getletter v)
  (let ((num 0) (value (vector-ref v 0)))
    (do ( (i 1 (1+ i)))
      ((= i (vector-length v)) (format "~A з ймовірністю ~A"  (string(string-ref "01234567890abcdefghijklnmopqrstuvwxyz" num)) value))
...
(define (compute)
  ...
  (if (= 0 NUMHID)
      ...
      (let ( (in (make-vector-my NUMIN)) (res 0) )
        (display "Введіть вектор:")
        (readvector in)
        (set! res (getoutput in))
        (newline)
        (display "Отриманий вихід: ")
        (print res)
        (newline)
...
(define (main)
  ...
  (print
   (format "Размір входу - ~a, розмір виходу - ~a, кількість шаблонів - ~a"
           NUMIN NUMOUT NUMPAT
           ))
  (newline)
  
  (train Input Output 0.0001 1500000 #t)
