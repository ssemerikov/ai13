...
(define NUMHID 100) ;размерность скрытого слоя
(define NUMPAT 0) ;количество шаблонов для обучения 
(define NUMOUT 100) ;размерность выхода
...
(define (menu)
  (display "Выберите:")
  (newline)
  (print "1. Обучение программы (ДОЛГО!!!)")
  (newline)
  (print "2. Общение")
  (newline)
  (print "3. Выход")
...
(define (Эллочка-говорит res)
  (let (
        (max (vector-ref res 0)) (index 0)
                                 )
    (do
        (
         (i 0 (+ 1 i))
         )
      ((= i 18)  (and (format "~A (с вероятностью ~A)" (list-ref словаричек index) max))  )
      (when (> (vector-ref res i) max)
        (set! max (vector-ref res i))
        (set! index i)
        )
      )
    )
...
(define (compute)
  (if (= 0 NUMHID)
      (print "Нейронная сеть не существует")
      ...
      (begin
        ...
        (let ([str (read-line)])
          (let ([res ""])       
        ;(string->vector s)
        (set! res (getoutput (string->vector str))) ;;Тут
        (newline)
        (print " Полученный выход: ")
        (display (Эллочка-говорит res))
        (newline)
...
(define (main)
  ...
  (train Input Output 0.0001 150000 #t)
